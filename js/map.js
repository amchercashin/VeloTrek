...
const VeloMap = (() => {
  ...
  const FS_STYLE =
    "filter:drop-shadow(0 0 2px #fff) drop-shadow(0 1px 2px rgba(0,0,0,.6))";
  ...
  const OfflineTileLayer = L.TileLayer.extend({
    createTile: function (coords, done) {
      ...
      OfflineTiles.getTile(key)
        .then((blob) => {
          if (blob) {
            const blobUrl = URL.createObjectURL(blob);
            tile.onload = () => URL.revokeObjectURL(blobUrl);
            tile.src = blobUrl;
            done(null, tile);
          } else {
            // Загрузка из сети
    ...
    const FullscreenControl = L.Control.extend({
      ...
      onAdd: function () {
        const container = L.DomUtil.create(
          "div",
          "leaflet-control leaflet-fullscreen-control",
        );
        fullscreenBtn = L.DomUtil.create(
          "a",
    ...
    const poiMarkers = routeData.pois.map((poi) => {
      return L.circleMarker([poi.lat, poi.lon], {
        ...
        `<b>${escapeHtml(poi.name)}</b>` +
          (poi.description
            ? `<br>${escapeHtml(poi.description).substring(0, 200)}`
            : ""),
      );
    });
